...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 *   Разбивает текст на строки, разделяющиеся \n
 *
 * * * * * * * * * * * * * * * * * * * * */
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 *
 *   Разбивает текст на условие и ответы (разделитель -- SEP).
 *   Сначала условие;
 *   Потом два сепаратора;
 *   Далее строки ответов, разделенные одним сепаратором.
 *
 * * * * * * * * * * * * * * * * * * * * * * * */
...
char *polynomial (char *buf, int n, ...)
{
	int count;
	int v[BUFSZ];
    char (*t)[BUFSZ] = (char(*)[BUFSZ])new char[100 * BUFSZ];
	count=n;
	va_list ap;
	va_start(ap, n);
	for(int i=0; i<n; ++i)
...
{
	int one = 1;
	simplifyfrac(r1,i1,one);
	simplifyfrac(r2,i2,one);
...
char *print_fracparts (char *buf, int n, char c[][BUFSZ], char d[][BUFSZ])
{
	...
	for(int i=0; i<n; ++i)
	{
		if (strcmp(c[i],"")!=0)
		{
			sprintf(t, "%s(%s)", (strcmp(coeff,"")==0?"":QString(QString(coeff)+"*").toStdString().c_str()), c[i]);
			strcpy(coeff,t);
		}
	}
	for(int i=0; i<n; ++i)
	{
		if (strcmp(d[i],"")!=0)
		{
			sprintf(t, "%s(%s)", (strcmp(div,"")==0?"":QString(QString(div)+"*").toStdString().c_str()), d[i]);
			strcpy(div, t);
		}
	}
	if (strcmp(div,"")==0)
...
char *chprintf (char *buf, const char *fmt, ...)
{
   va_list argptr;
   int cnt;
...
char *catprintf (char *buf, const char *fmt, ...)
{
   va_list argptr;
   int cnt;
   char buf1[BUFSZ];
