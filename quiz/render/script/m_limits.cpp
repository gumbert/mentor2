...
void lim121t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->inf,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=a1/b1\np1=q1,p1>p2>p3,q1>q2>q3");
	sprintf(text,"lim(x->inf,(%s)/(%s))=%s",
...
void lim122t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->inf,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=0\np1<q1,p1>p2>p3,q1>q2>q3");
...
void lim123t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->inf,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=inf\np1>q1,p1>p2>p3,q1>q2>q3");
...
void lim124t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->0,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=a3/b3\np3=q3,p1>p2>p3,q1>q2>q3");
...
void lim125t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->0,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=inf\np3<q3,p1>p2>p3,q1>q2>q3");
...
void lim126t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
	...
	sprintf(src, "lim(x->0,(a1*x^p1+a2*x^p2+a3*x^p3)/(b1*x^q1+b2*x^q2+b3*x^q3))=0\np3>q3,p1>p2>p3,q1>q2>q3");
...
void lim3410t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
    ...
    sprintf(text, "lim(x->%d, tg((pi*x)/(%d))*sin((x%s)/2))=%d/pi", a,2*a,(-a>0?(QString("+")+(-a)).toStdString().c_str():QString::number(-a).toStdString().c_str()),-a);
...
void lim491t (const range& r, char *text, char answ[][BUFSZ], char *src)
{
		...
		do{
			...
			c=rndr(r);
			d=rndr(r);
		}while (!(a!=0 && abs(c) < abs(d) && d!=0 && ((c<0 && d<0)||(c>0 && d>0)) ));
		sprintf(text, "lim(x->%s+0, !((%s)/(%s))&String(\"^\")&!(1/(%s)))=0",
				fraction(buf[9], c*c*c, 1, d*d*d),
				polynomial(buf[0],2, a,"x^2",b,"x"),
				poly2ns(buf[1],2, c,1,1,1,"x", a*c*c*c+b*d*d*d-c*c*c*c,d*d*d,1,1,""),
				poly2ns(buf[2],2, 1,1,1,1,"(x^(1/3))", -c,d,1,1,"")
		);
