...
{
    char (*buf)[BUFSZ] = (char(*)[BUFSZ])new char[15 * BUFSZ];
	int a,b,c;
	do{
		a=rndr(r);
		b=rndr(r);
...
void deriv603 (const range &rr, char task[BUFSZ], char answ[5][BUFSZ], char src[BUFSZ])
{
	...
	sprintf(answ[0], "y`=((%s)*(%s)/(%s))*(%d/(%s)*sqrt(x))-((%d*x^2)/(%s))-((%d)/(%s)))",
		memb(buf[0], 1,1,m,1, asqxpb, false),
		memb(buf[1], 1,1,r,1, x3pq,  false),
		memb(buf[2], 1,1,s,1, xpp, false),
		m*a, asqxpb,
	...
	sprintf(answ[2], "y`=%d/(2*(%s)*sqrt(x))-((%d*x^2)/(%s))-((%d)/(%s))",
		m*a, asqxpb,
		3*r, x3pq,
		s, xpp
	);
	sprintf(answ[0], "y`=((%s)*(%s)/(%s))*(%d/(%d*(%s)*sqrt(x))-((%d*x^2)/(%s))+((%d)/(%s)))",
		memb(buf[0], 1,1,m,1, asqxpb, false),
		memb(buf[1], 1,1,r,1, x3pq,  false),
		memb(buf[2], 1,1,s,1, xpp, false),
		a, a, asqxpb,
...
void deriv71 (const range &r, char task[BUFSZ], char answ[5][BUFSZ], char src[BUFSZ])
{
	...
	catprintf(task,
		n==1?
			chprintf(buf[2], "%s", "y=(%s)/(%s)")
		  : n==2?
			chprintf(buf[2], "%s", "y=(%s)/sqrt(%s)")
...
void deriv715 (const range& r, char task[BUFSZ], char answ[5][BUFSZ], char src[BUFSZ])
{
	...
	catprintf(task,
		n==2?
			chprintf(buf[2], "%s", "y=((%d)^(%s))/sqrt(%s)")
		  : chprintf(buf[2], "%s", "y=((%d)^(%s))/root(%s,%d)"),
		b,
